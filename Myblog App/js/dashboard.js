var API_BASE_URL = (typeof window !== "undefined" && (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1"))
...
function openAdminBlogPreview(blogId) {
    ...
    overlay.innerHTML = '<div style="background:var(--bg-card, #FFFFFF);color:var(--text-primary, #0F172A);width:100%;max-width:760px;max-height:90vh;border-radius:24px;overflow-y:auto;box-shadow:0 25px 60px rgba(0,0,0,0.4);position:relative;display:flex;flex-direction:column;border:1px solid var(--border-color, rgba(99,102,241,0.2));">' +
        ...
        (fullVideo ? '<video src="' + esc(fullVideo) + '" controls style="width:100%;max-height:360px;border-radius:16px;margin-bottom:24px;background:#000;box-shadow:0 6px 20px rgba(0,0,0,0.2);"></video>' : (fullImage ? '<img src="' + esc(fullImage) + '" alt="preview" style="width:100%;max-height:340px;object-fit:cover;border-radius:16px;margin-bottom:24px;box-shadow:0 6px 20px rgba(0,0,0,0.2);">' : '')) +
